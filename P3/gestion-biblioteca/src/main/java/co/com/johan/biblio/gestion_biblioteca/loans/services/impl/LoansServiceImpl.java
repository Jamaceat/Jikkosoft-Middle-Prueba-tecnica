...
public class LoansServiceImpl implements LoansService {
    ...
    public List<LoanEntity> registerLoans(RegisterLoansR loans) throws BadRequestException {
        ...
        if (books.stream().distinct().count() != loans.booksId().stream().distinct().count()) {
            String remainingBooks = loans.booksId().stream()
                    .filter(bookId -> books.stream().allMatch(currentBook -> currentBook.getId() != bookId))
                    .map(String::valueOf)
                    .collect(Collectors.joining(","));
            throw new BadRequestException(
                    "hay uno o varios libros que no estan disponibles o no existen con los Ids: " + remainingBooks);
    ...
    public List<LoanEntity> returnLoans(ReturnLoansR loans) throws BadRequestException {
        ...
        log.info("member loanIds {}", memberLoans.stream().map(loan -> loan.getBookId()).map(String::valueOf)
                .collect(Collectors.joining(",")));

        List<LoanEntity> validLoans = memberLoans.stream()
                .filter(loan -> loans.LoanIds().stream().anyMatch(compareLoan -> loan.getId() == compareLoan)).toList();

        if (validLoans.stream().map(loan -> loan.getId()).distinct().toList().size() != loans.LoanIds().size()) {
            String invalidLoans = loans.LoanIds().stream()
                    .filter(loan -> memberLoans.stream().allMatch(compareLoan -> loan != compareLoan.getId()))
    ...
    private List<BookEntity> recalculateBooksReturn(List<Long> distinctBookIds, List<BookEntity> books)
        ...
        List<BookEntity> distinctBooks = distinctBookIds.stream()
                .map(bookId -> books.stream().filter(book -> book.getId() == bookId).findFirst().orElseThrow())
                .toList();
        ...
        if (distinctBooks.stream().anyMatch(book -> book.getAvailableAmount() > book.getTotalAmount())) {
            ...
            String errorBooks = distinctBooks.stream().filter(book -> book.getAvailableAmount() > book.getTotalAmount())
                    .map(book -> String.format("(%d;%d)", book.getAvailableAmount() - book.getTotalAmount(),
                            book.getId()))
                    .collect(Collectors.joining(","));
    ...
    private List<BookEntity> recalculateBooksLoan(List<Long> distinctBookIds, List<BookEntity> books)
        ...
        if (distinctBooks.stream().anyMatch(book -> book.getAvailableAmount() < 0)) {
            String errorBooks = distinctBooks.stream().filter(book -> book.getAvailableAmount() < 0)
                    .map(book -> String.format("(%d;%d)", Math.abs(book.getAvailableAmount()), book.getId()))
                    .collect(Collectors.joining(","));
